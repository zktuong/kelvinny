...
trainScSimilarity <- function(train_data, train_cell_type, train_genes = NULL, 
    standardize = TRUE, nfolds = 10, a = 0.75, l.min = TRUE, multinomial = FALSE, 
    ...
    if (is.null(train_genes)) {
        ...
        if (multinomial == FALSE) {
            ...
            for (label in labels) {
                ...
                fit[[L]] = tryCatch(glmnet::cv.glmnet(train_dat, celltype, 
            ...
            for (i in 1:length(fit)) {
                ...
                fit_out <- fit_out[fit_out[, 1] != 0, , drop = FALSE]
                cat(sep = "\n")
                print(paste0("Best genes for ", levels(train_cell_type)[i]))
                print(fit_out)
            }
            return(fit)
        }
